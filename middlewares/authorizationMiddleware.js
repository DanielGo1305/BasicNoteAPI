 ...
 */
function authorize(req, res, next) {    // HARUS punya 3 param (req : request user ke server, res : response server trhadap req, next : mempersiapkan middleware untuk digunakan di routes lain)
    /**
    ...
    const authorization = req.headers.authorization   // proses verifikasi apakah ada token yang dimasukkan / tidak
    if (!authorization) {
        res.send("No token detected") //response jika tidak ada token yang dimasukkan
    }
    // 👇 verify that the token is using bearer token format
    else if (!authorization.startsWith('Bearer')) {  //untuk memverifikasi token menggunakan format bearer token (bearer ashhs1232dsa)
        res.send('Wrong token format') //response jika format token salah (bukan bearer)
    }
    else {
        ...
        const splittedAuthorization = authorization.split(' ') // memisah string berdasarkan spasi ("bearer" "sdsgdhg23214")
        ...
        if (splittedAuthorization.length < 2) { //jika setelah di split bearer token length <2
            // 👇 If the token format is wrong then tell the client that the token is wrong
            res.send("Wrong token format") //responsenya wrong token format(karena bearer token length nya 2 setelah di split)
        } else {
            // 👇 use the <token> from splitted authorization
            const token = splittedAuthorization[1] // mengeleminasi string bearer & mengambil token aja
            // 👇 use parseJwt function to break down token information
            const user = parseJwt(token)
            if (!user) {  //memverifikasi token apakah expired atau tidak
                // 👇 if the parseJwt returns null, tell that the token is expired
                res.send("Expired token") //response jika token expired
            } else {
                // 👇 if its returning the token information, add a new user property in req object and fill it with token information
                req.user = user //jika token terverifikasi ada di database, akan membuat properti baru (token) dan valuenya adalah token yang dibuat nanoId
                // 👇 tell express that we're done with this middleware and it's ready to be handled by the route who use this middleware
                next() //memberitahu middleware ini siap digunakan oleh routes lain yang membutuhkan middleware ini
            }
        }
    }
}
